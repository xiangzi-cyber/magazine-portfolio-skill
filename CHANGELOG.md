...
- 新增 P0「文案冻结保护」:当用户已确认文案、当前任务只是"进图 / 加图 / 替换图"时,进入 Image-only Mode。
- 新增 Image-only Mode 执行边界:只做入库命名、压缩处理、替换图片占位、验证加载、更新素材清单。
- 新增进图前落位核查要求:"哪张图 → 哪个占位 → 用途 → 状态"必须先对齐再执行。
- 新增 P0「截图插入双模式」:默认使用截图证据板,用当前主题背景、留边、窗口/细边框和轻阴影承托截图。

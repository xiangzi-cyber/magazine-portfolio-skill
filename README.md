...
![规则可应用架构总览](./assets/rule-applicable-architecture.png)

这张图的生成提示词留档在 [`prompts/01-rule-applicable-architecture.md`](./prompts/01-rule-applicable-architecture.md)，后续可以按同一信息结构重出图。
...
下面这张图是更精确的运行协议，方便维护者判断一条新规则该挂到哪里：

![规则可应用运行图](./assets/rule-applicable-workflow.svg)
...
├── assets/
│   ├── template.html
│   ├── rule-applicable-architecture.png
│   └── rule-applicable-workflow.svg
├── prompts/
│   └── 01-rule-applicable-architecture.md
└── references/
    ├── rule-gates.md
    ├── workflow.md
    ├── 7-questions.md
...
| `assets/rule-applicable-architecture.png` | GPT 生成的 README 架构总览图 |
| `assets/rule-applicable-workflow.svg` | 可维护的精确运行协议图 |
| `prompts/01-rule-applicable-architecture.md` | 架构总览图的生成提示词留档 |

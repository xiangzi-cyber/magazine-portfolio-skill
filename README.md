...
**Magazine Portfolio Skill · AI 作品集生成工作流系统**

面向设计师、产品经理和独立创作者的求职作品集制作场景，构建了一套从素材解析、证据归位、叙事组织到单文件 HTML 交付的 AI 工作流系统。系统将聊天记录、项目截图、复盘文档等非结构化材料转化为招聘方可独立阅读的长滚动作品页，并通过规则门禁、品牌组件和自动化校验，降低 AI 生成内容常见的空泛化、视觉漂移和模板失效风险。
...
- 建立 G0-G7 门禁规则，将“不编造、不越权、模板激活、图片证据、浏览器验收”等质量要求转化为可执行的系统约束。
- 沉淀 Explorer Violet 品牌系统、Explorer Voice 五层字体系统、`x-` 组件库和 Expedition Profile 机制，使不同项目页既保持统一识别，也能表达各自差异。
- 开发 22 项自动化页面校验器，覆盖色彩、字体、组件、叙事结构、正文可读性和视觉边界，降低 AI 生成作品集时的漂移风险。
- 基于火种车、春晚、KA21 等真实项目页抽象通用方法，将一次性页面制作经验沉淀为可复用的 Skill 系统。
...
早期 guizang 给了它“网页化杂志表达”的启发：单文件 HTML、杂志感版式、电子墨水气质和图文节奏。随着 v4/v5 重构，这个 skill 的核心能力已经转向一套独立工具链：
...
> 构建面向求职作品集的 AI 工作流系统，将非结构化项目材料转化为招聘方可独立阅读的长滚动网页，并通过规则门禁、品牌组件和自动化校验降低内容与视觉漂移。
